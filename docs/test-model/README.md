...
Synthetic certification 只证明合同与失败路径，不能冒充 controlled-real minimum certification。独立的受控 `REMOTE_DEEPSEEK` 与 `LOCAL_LLAMACPP` minimum certification 已在冻结的 `HARNESS-01` route 上 PASS，[remote 公开证据](../model-certification/remote-deepseek-controlled-real-01.json)与 [local 公开证据](../model-certification/local-llamacpp-controlled-real-01.json)均不含 credential 值或私有路径。受控流程累计 3 次真实模型调用：既有 remote/network 2 次、local 1 次；formal remote 与 local certification 各精确绑定 1 次成功调用。`GGUF-04` locator、完整摘要、metadata 和 `LLAMACPP-01` compatibility/startup/invocation/shutdown probes 均已验证，且没有导出 locator、下载、猜测或替代资产。`DEFER-003` 未关闭，不声称 full GM/Player、Campaign、long-context、tool-use、production performance 或 production readiness。
