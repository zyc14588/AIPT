...
`AIPT-MVP-B002 = IN_PROGRESS`：当前唯一施工批次，精确 Base `411bf2997cd0f10ba1a022ac687d27a1bd19eb36`（tree `d1daaeede13a2ba07c3b528c1792ef9fd5600a63`），`GLOBAL_WIP = 1`。Candidate 实现 game-neutral Deterministic Run Core 的 action transaction、authoritative state、versioned/domain-separated RNG、seed commitment、invariants、derived projection、PostgreSQL ledger atomicity 与 fail-closed replay；不实现 Agent 编排、model gateway、真实模型调用、真实桌测或 qualification Run。`merge_authorized = false`、`closeout_authorized = false`；下一项 `AIPT-MVP-B003` 为 `NOT_STARTED` / `NOT_AUTHORIZED`。`M0 Development Pass = GRANTED` 继续有效，MVP Development Pass 仍为 `NOT_GRANTED`；生产/发行资格仍为 `NOT_GRANTED`，真人等价仍为 `NOT_CLAIMED`，平台集成仍为 `FROZEN_WAITING_M1_ENGINE`。
