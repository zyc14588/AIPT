...
`AIPT-MVP-B004 = IN_PROGRESS`：Owner 授权 Base 为 B003 closeout `98591311c4872cdc5f091e23fba1acb500ad4599`（tree `a02ffea60c59d7187975f175875fa108c78d3cac`）。施工已实现版本化 Model/Sampling Profiles、per-role immutable binding、完整 execution tuple、`REMOTE_DEEPSEEK` / `LOCAL_LLAMACPP` 闭集、固定 DeepSeek Harness ACP gateway、credential/egress/context/local-process 安全边界，以及 MODEL/HARNESS launcher gates；IPC 仍是首个阻塞 gate。当前 `GLOBAL_WIP = 1`，下一项 `INT-AIPT-UNREGISTERED-MVP-001` 未授权且未启动。`HARNESS-01`、Owner 批准的 credential reference 与 route 已冻结；受控 `REMOTE_DEEPSEEK` 与 `LOCAL_LLAMACPP` minimum certification 均已 PASS，[remote 公开证据](../model-certification/remote-deepseek-controlled-real-01.json)与 [local 公开证据](../model-certification/local-llamacpp-controlled-real-01.json)均不含 credential 值或私有路径。受控流程累计 3 次真实模型调用：既有 remote/network 2 次、local 1 次；formal remote 与 local certification 各精确绑定 1 次成功调用，local 首次 readiness 失败发生在调用前。`GGUF-04` locator 已按批准 root、canonical target、完整 SHA-256 与 metadata 验证但未导出，`LLAMACPP-01` compatibility、受管启动、Harness role invocation 与有界关闭/失败探针均 PASS；没有下载、猜测或替代资产，`DEFER-003` 保持未解析。公共 CI 真实模型/网络调用和 secret requirement 均为 0，真实 playtest 与 qualification Run 均未执行；Candidate 身份由最终提交/Tree 与公开 CI Stage Review 确定，B004 未 merge 或 closeout。`M0 Development Pass = GRANTED` 继续有效，MVP Development Pass、生产/发行资格仍为 `NOT_GRANTED`，真人等价仍为 `NOT_CLAIMED`。
