...
5. 迭代 5 Go 协议契约消费者：依赖自由的 Go 包 `internal/protocol`（仅 Go 标准库，`go.mod`/`go.sum` 不变）——严格 fail-closed JSON-RPC 2.0 信封解码、确定性 canonical JSON/SHA-256 与 Node 预言机一致、纯投影/可见性语义与 hidden-leak 突变绑定、共享夹具端到端兼容测试与权威 Schema `$defs` 漂移测试；`package.json` 新增 `test:protocol-go`（恰为 `go test ./internal/protocol -count=1`）；`constants.mjs` 登记 `internal/protocol/**` 与 `internal/toolchainsmoke/doc.go` 准入、移除 `internal/protocol/` 禁止前缀；Go 测试接入公共 CI workflow（B002 聚焦工作流演进）留待后续迭代；
