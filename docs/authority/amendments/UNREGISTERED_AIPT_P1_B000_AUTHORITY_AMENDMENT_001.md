...
The replacement Candidate is the exact two-ordinary-commit lineage on branch `task/UNREGISTERED-AIPT-P1-B000-AUTHORITY-AMENDMENT-001-R1` rooted directly at the exact base merge. The first R1 attempt, `2296eb5ec90c64976b014663d04faff4530b4c48` (tree `f05010b18f63c9833e5ed2c2d7f1cdad212fe844`, CI `33039836247`), is preserved with real conclusion `failure`; it exposed the missing GitHub-push wording variant and is not treated as recoverable or PASS. The legal Amendment merge must have the final approved replacement Candidate as second parent and a tree equal to that Candidate tree. The closeout successor must then be the direct, single-parent child of that accepted Amendment merge. A second-generation or unrelated successor is not eligible.
...
- F1 binds `scripts/ci/validate/p1-b000-authority.mjs` to SHA-256 `f5ed47898ad13b193cd685ae9649c18cada3a6fb5893c1810867c91869ad8c7c`, structured result `FAIL`, and the exact stable candidate-only topology/scope failures caused by the post-base governance lifecycle. The frozen branch failure has context-specific text (`local authority checkout…` locally, `authority Candidate push…` on GitHub push, and no branch failure on `pull_request` because the frozen code only branch-gates push); the classifier binds that variant to the actual event. Its reported path inventory must equal the exact Git lifecycle diff; any additional `FAIL` rejects.
