...
验证器对当前 **20 条**清单记录逐一校验期望的 SPDX 许可值与精确 kind：一方集合恰为 {`AIPT`, `@aipt/adapter-sdk`}（均为 `first_party`/MIT），B001 批准的精确工具/CI/基础设施集合原样保留（actions/checkout、actions/setup-go、actions/setup-node → `ci_action`/MIT；go、node、pnpm → `toolchain`；postgresql、docker-library/postgres → `infrastructure_image_component`；postgresql-docker-official-image → `infrastructure_image`/NOASSERTION；golang.org/x/vuln → `supply_chain_tooling`/BSD-3-Clause），六个第三方 Go 运行时模块保持 kind `third_party_go_runtime`、role `runtime_dependency`；其中 `selected_by_batch = AIPT-M0-B003` 保留最初引入历史，B004 的版本/漏洞再资格化由独立的封闭 provenance 对象记录：
...
**精确集合模型**：当前记录集合必须恰为 20 个 identity（历史 18 个 + x/mod 与 x/tools 两个 selected-graph tooling identity）；任何未登记第三方身份一律拒绝。SDK 记录携带真实 B002 元数据，六个运行时记录保留真实 B003 选择历史，B004 requalification 记录精确版本、角色、checksum、许可与触发原因；licenses.json 文件顶层 `selected_by_batch` 继续锁定不可变历史 `AIPT-M0-B001`。机器负向探针覆盖 vulnerable/below-fixed/newer x/text、pgx 漂移、缺 checksum/license/provenance、MVS 版本/角色漂移及无关 module 注入，磁盘文件从不被探针改写。
...
**B004 当前身份**：生成器输出 `AIPT-M0-B004-supply-chain-sbom`，AIPT 根版本为 `M0-B004`，确定性 `created` 为 `2026-08-20T00:00:00Z`，内容寻址 namespace 为 `https://github.com/zyc14588/AIPT/spdx/aipt-m0-b004/<hash>`。包集合精确为 20 个：B003 的 18 个身份原样保留并更新 x/sync/x/text 的当前版本，另加入 x/mod v0.37.0 与 x/tools v0.47.0 两个 selected-graph tooling 包。运行时图仍为 AIPT→pgx→五个 indirect 模块；x/text 另对其 selected graph 的 x/sync/x/mod/x/tools 建立 `DEPENDS_ON`，x/mod 与 x/tools 以 `BUILD_TOOL_OF AIPT` 明确排除在 runtime closure 外。验证器同时拒绝 vulnerable/below-fixed/newer x/text、缺失 tooling 包/checksum/关系及 tooling 被误标为 runtime。
