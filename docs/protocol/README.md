...
## 10. Adapter SDK（B002 迭代 4/4B/4C）
...
- 运行：`pnpm --filter @aipt/adapter-sdk test`（`node:test`/`assert`，确定性、密封、无外部网络；**107 项测试**——迭代 4 的 53 项与迭代 4B 的 90 项全部保留并新增 17 项迭代 4C 聚焦测试）；`pnpm run check:adapter-sdk`（机器门禁，**81 个负向探针**：62 个 fail-closed 行为探针（含全部已确认误接受修复探针）+ 11 个内存漂移探针 + 6 个零调用/无文档触碰探针 + hidden-leak 突变探针 + 未来 wire 错误码探针；已并入 `pnpm run check`）。
