...
## 11. Go 协议契约消费者（B002 迭代 5/5B/5C）

- **依赖自由、纯协议 Go 包**：[internal/protocol](../../internal/protocol/)（模块 `github.com/zyc14588/AIPT`，`go 1.26.x` / `toolchain go1.26.6`——B003 安全重资格，见 [../authority/DEFERRED_PARAMETERS.md](../authority/DEFERRED_PARAMETERS.md)）。仅用 Go 标准库；**`go.mod`/`go.sum` 不变、零第三方依赖**。生产 API 纯函数化：无文件 I/O、无环境访问、无进程/网络/socket/数据库、无服务循环/goroutine worker/模型调用——不是 Core 运行时、规则引擎、服务、启动器或 Adapter。
...
- **运行**：`pnpm run test:protocol-go`（`package.json` 持久脚本，恰为 `go test ./internal/protocol -count=1`）；共 **205 项测试**全绿，其中 **116 项负向用例**（畸形 jsonrpc、未知协议/模式版本、缺 params、result+error 同时/皆无、未知请求/通知方法、任意根对象、未知字段、顶层与嵌套重复键、尾随 JSON、缺失/未知/空可见性、重复字段/席位、未知席位、投影值/元数据漂移与遗漏授权字段、id 字符串/数字往返与闭区间边界、不安全/小数/null/负零 id 拒绝、错误 code/data 无效、夹具身份失配、突变元数据漂移、canonicalization 拒绝重复/尾随/不安全/负零输入，以及迭代 5B 新增的 18 项聚焦负向用例与迭代 5C 新增的 15 项聚焦负向/对抗用例——孤立代理混同拒绝、非法 UTF-8 拒绝、空 fields/重复源 field/未知源席位投影门禁等）。测试密封、无副作用、不改动任何受跟踪的 schema/夹具文件。
...
- **迭代 5C 修复（Codex 独立探针复现的四个 Go 契约缺口，全部以聚焦回归测试关闭）**：
  1. **字符串 `RequestID` 精确值**：`requestIDFromNode` 不再把字符串 id 折叠成 Go 替换符——id 以**规范引用 JSON 文本**保存解析器的 UTF-16 码元序列，`MarshalJSON` 输出 Node 兼容的精确值（孤立代理为小写 `\ud800`/`\udc00`，合法对/字面星界标量保持同值），`Equal` 按精确字符串值比较（孤立 D800 ≠ U+FFFD、转义合法对 == 字面标量、同值异拼写相等、字符串 vs 数字恒不相等），表示是不可变字符串、无暴露可变底层切片；1..128 字符按 JavaScript 码点语义对 BMP/星界对/孤立码元一致生效并有 128/129 边界探针；`String()` 文档与行为诚实化——数字 id 返回十进制形式，字符串 id 仅返回有损 Go Unicode 视图（孤立代理显示为 U+FFFD，绝不自称精确），request/response 信封级孤立代理 id 往返有端到端测试。
  2. **`NewStringID` 非法 UTF-8 确定性拒绝**：调用方提供的非法 UTF-8 Go 字符串不是忠实 JSON 字符串边界，`NewStringID` 现以类型化 `AIPT_ID_INVALID` 拒绝（绝不静默重写为 U+FFFD）；真实 U+FFFD、控制字符与星界标量保持有效，且 `NewStringID` 创建的合法 id 与解析 id 共享同一精确/规范表示与相等规则。
  3. **空 fields = 缺失字段**：`CheckStateMetadata` 将 `len(state.Fields) == 0`（nil 或 present-but-empty 非 nil 切片）视为 schema `fields.minItems = 1` 违约并返回稳定 `AIPT_STATE_MISSING_FIELDS`。
  4. **全量投影门禁覆盖源 state 元数据**：`CheckProjection` 在投影专属原因之前**确定性地先运行 `CheckStateMetadata`**——重复源 field_id 产生 `AIPT_STATE_DUPLICATE_FIELD_ID`、未知源授权席位产生 `AIPT_VISIBILITY_UNKNOWN_SEAT`、空 fields 产生 `AIPT_STATE_MISSING_FIELDS`，`ValidateProjection` 因而以首个稳定相关原因失败而非放行；组合/对抗测试证明从同一缺陷 state 逐字复制的投影（授权集合仍匹配）无法掩盖源缺陷；全部既有合法夹具结果、投影专属原因、数学集合授权比较、身份与 nil 行为、hidden-leak 突变在干净源 state 上的唯一 `AIPT_VISIBILITY_UNAUTHORIZED_FIELD` 结果逐一保留。
