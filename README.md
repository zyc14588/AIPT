...
| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IN_PROGRESS`，`current_batch = AIPT-M0-B006`，`GLOBAL_WIP = 1`；权威快照 `AIPT-M0-B006-CONSTRUCTION-001` |
...
`AIPT-M0-B006` = **IN_PROGRESS**：从上述 B005 closeout 基线建设公开 Evidence/Audit v1 Schema、最小原生 `RAW_CAPTURE` exporter/verifier、只读 PostgreSQL ledger source、合成夹具及 fail-closed CI 门禁；不建设 `AUDIT_READY`/`AUDIT_RESULT` generator，不实现签名、加密或分块。当前 `construction = IN_PROGRESS`、`current_batch = AIPT-M0-B006`、`GLOBAL_WIP = 1`；下一串行项 `UNREGISTERED-AIPT-P0-B002` 为 `NOT_AUTHORIZED`（`next_batch_authorized = false`，`next_batch_started = false`）。`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`（`unfreeze_authorized = false`）。
...
- Evidence/Audit（B006，施工中）：[docs/evidence/README.md](docs/evidence/README.md) → 公开三阶段 Schema [aipt-evidence.schema.json](schemas/evidence/v1/aipt-evidence.schema.json) 与最小原生 Go [`RAW_CAPTURE` exporter/verifier](internal/evidence)；只读复用 B003 `VerifyStream`，canonical JSON 复用 B002 `internal/protocol.CanonicalJSON`，合成 golden 位于 [testdata/evidence/v1](testdata/evidence/v1)。`AUDIT_READY`/`AUDIT_RESULT` generator、签名、加密与分块均未实现。
