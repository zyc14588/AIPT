...
- Go 协议契约消费者（B002 迭代 5）：[internal/protocol](internal/protocol)（仅 Go 标准库，零依赖；严格 JSON-RPC 2.0 信封解码、确定性 canonical JSON/SHA-256、投影/可见性语义、共享夹具兼容与 schema 漂移测试；运行 `pnpm run test:protocol-go`，即 `go test ./internal/protocol -count=1`）
