...
| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；施工状态 `IDLE_WAITING_NEXT_BATCH`，`current_batch = NO_ACTIVE_BATCH`，`GLOBAL_WIP = 0`；权威快照 `AIPT-M0-B004-CLOSEOUT-001` |
...
`AIPT-M0-B004` 已 `MERGED/CLOSED`；Launcher、共享配置基础、Core lifecycle shell 与 B003 PostgreSQL/迁移接线均由固定 implementation merge/tree 和成功 repair CI 验证。当前 `current_batch = NO_ACTIVE_BATCH`、`GLOBAL_WIP = 0`，施工状态 `IDLE_WAITING_NEXT_BATCH`。M0 串行链下一批 `AIPT-M0-B005` 为 `AUTHORIZED_TO_PREPARE`（`next_batch_authorized = true`，`next_batch_started = false`）；只获准准备，本 closeout 不启动 B005 实施。
...
- 工具链与供应链：[docs/supply-chain/README.md](docs/supply-chain/README.md) 与 [tools/toolchain.lock.json](tools/toolchain.lock.json)（B003 原始 pgx v5.10.0 运行时闭包 go=6/pnpm=0 与 18 身份清单保留为历史；B004 因 GO-2026-5970 将 x/text 精确再资格化到 v0.39.0，当前许可证/SBOM 为 20 身份，其中 x/mod 与 x/tools 仅为 selected-module-graph tooling；pgx v5.10.0、Go 1.26.6 与冻结 `policy.json` 均不变）
