...
| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IN_PROGRESS`，`current_batch = AIPT-MVP-B000`，`GLOBAL_WIP = 1`；权威快照 `AIPT-MVP-B000-CANDIDATE-001` |
...
`AIPT-MVP-B000` = **IN_PROGRESS**：Owner Authority `AIPT-MVP-B000-START-001`；精确 Base 为 M0 closeout `c617f3c6ab3e56ac88f228ed4825e751537fc1f0`（tree `95a8d2980c5a6aa44f3db67c66f07ff008ff3491`，CI `32828913767` success）。本批次只安装 13 项 MVP 权威串行图、Candidate 生命周期与 fail-closed CI validator，不实现 Run engine、不调用真实模型、不执行真实桌测或资格 Run。`next_serial_batch = AIPT-MVP-B001`，但 `next_batch_state = NOT_AUTHORIZED`、`next_batch_authorized = false`、`next_batch_started = false`；所有更晚批次均为 `NOT_STARTED`。
...
**设计基线已冻结。** M0 B001–B008 均已合并关闭；协议、SDK、PostgreSQL 存储、Runtime Shell、Harness Adapter、Evidence/Audit 与安全的 loopback-only Web Dashboard 实现身份均保持不可变。AIPT-MVP-B000 仅施工治理/CI bootstrap，不建设 queue/run/status backend、report generator、socket/model runtime 或完整动作管线；`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 保持耐久名称 `AIPT M0 CI`，在两个 toolchain matrix 实例无条件运行 `check:mvp-bootstrap`，同时保留 B008 `check:m0-development-pass` 与全部历史聚焦门。`pnpm run check` 是 M0 历史门 + AIPT-MVP-B000 bootstrap 的聚合门禁（报告 schema `aipt.public.mvp-b000-validator-run/v1`）。
