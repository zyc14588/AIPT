...
| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IN_PROGRESS`，`current_batch = AIPT-MVP-B001`，`GLOBAL_WIP = 1`；权威快照 `AIPT-MVP-B001-CANDIDATE-001` |
...
`AIPT-MVP-B001` = **IN_PROGRESS**：Owner Authority `AIPT-MVP-B001-START-001`，授权仅为 `IMPLEMENT_AND_FREEZE_CANDIDATE_ONLY`；分支 `task/AIPT-MVP-B001` 从精确 B000 closeout Base `64b5692971bbe687884ec34bd6417fe803987ae9`（tree `1a6feabb1796af9f66fd78fc842f249ec03a5251`）线性施工。范围只包括版本化声明式 Test Plan、不可变且内容寻址的 Run Manifest，以及 PostgreSQL 18.4 权威串行 Queue/Lease/Attempt 持久层；不包括 Run Core、Agent 编排、产品模型调用、真实桌测、合并或 closeout。当前 `GLOBAL_WIP = 1`；下一项 `UNREGISTERED-AIPT-P1-B000` 仍为 `NOT_STARTED` / `NOT_AUTHORIZED`，`next_batch_authorized = false`、`next_batch_started = false`。
...
**设计基线已冻结，B001 施工边界严格受限。** M0 B001–B008 与 AIPT-MVP-B000 均已合并关闭；既有协议、SDK、账本、Runtime Shell、Harness Adapter、Evidence/Audit 与 loopback-only Web Dashboard 身份保持不可变。AIPT-MVP-B001 只新增 Test Plan / Run Manifest / Queue-Lease-Attempt 权威合同；queue backend 不等于 Run engine，且 report generator、socket/model runtime、Agent 编排与完整动作管线仍未建设。`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
