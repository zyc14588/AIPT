...
**设计基线已冻结。** B001 已合并关闭（工具链骨架、公共 CI、供应链门禁）；B002–B006 的协议、SDK、PostgreSQL 存储、Runtime Shell、Harness Adapter 与 Evidence/Audit 历史均保持不可变。B007 仅新增 loopback-only 本地 Web Host 与只读 Dashboard，不建设 queue/run/status backend、report generator、socket/model runtime 或完整动作管线；`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 以耐久名称 `AIPT M0 CI` 在 `ubuntu-24.04` 与 `ubuntu-26.04` 两个合格 runner 上保留全部既有聚焦命令，并显式运行 B007 strict Web validator、TypeScript Dashboard tests、Go Web tests 与真实动态 loopback smoke；独立 storage job 继续在 digest-pinned、loopback-only PostgreSQL 18.4 上运行既有集成与 race。四个 job definition 经 toolchain 双矩阵形成五个必需 job instance。`pnpm run check` 是 B001–B007 聚合门禁（报告 schema `aipt.public.b007-validator-run/v1`）。
...
- Web（B007，施工中）：[docs/runtime/README.md](docs/runtime/README.md) → Go Host [`internal/web`](internal/web)、公开 DTO Schema [aipt-web.schema.json](schemas/web/v1/aipt-web.schema.json) 与零依赖 TypeScript [`@aipt/web-ui`](packages/web-ui)。机器门禁 `pnpm run check:web-ui`，UI/Go/smoke 分别运行 `pnpm run test:web-ui`、`pnpm run test:web-go`、`pnpm run smoke:web-ui`。
