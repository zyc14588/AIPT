...
**B004 Runtime Shell** 已形成施工实现：Go Launcher 固定执行 `CONFIG → POSTGRESQL → MIGRATIONS → MODEL → HARNESS → CORE → IPC → WEB`，真实路径完成严格配置加载、PostgreSQL Ping 与 B003 `MigrateUp` 后，在未实现的 `MODEL` 门禁以 `AIPT_LAUNCH_GATE_NOT_IMPLEMENTED` 失败关闭，绝不把 launch plan 冒充 runtime ready。新增 [`internal/config`](internal/config) / [`schemas/config/v1`](schemas/config/v1) 共享配置基础、[`internal/core`](internal/core) lifecycle shell、[`internal/launcher`](internal/launcher) 门禁机与 [`cmd/aipt`](cmd/aipt) 最小 CLI；SIGINT/SIGTERM 传播取消，启动失败与取消均执行独立有界的逆序清理并保留根错误。B004 没有新增第三方依赖，继续复用 B003 已资格化的 pgx v5.10.0 闭包；运行边界与配置示例见 [`docs/runtime/README.md`](docs/runtime/README.md)。本段是 B004 `IN_PROGRESS` 施工说明，不是 Candidate、merge 或 closeout 声明。
