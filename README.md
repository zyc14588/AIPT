...
## 当前状态（as of 2026-08-26）

| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IDLE_WAITING_NEXT_BATCH`，`current_batch = NO_ACTIVE_BATCH`，`GLOBAL_WIP = 0`；权威快照 `AIPT-MVP-B001-CLOSEOUT-001` |
...
`AIPT-MVP-B001` = **MERGED_CLOSED**：Owner-approved Candidate `85ef3489405694cf0764867a97fb21b09fda5894`（tree `44a885569c59c428fb173e0847dc49a8111b526c`，Candidate CI `32932281680` success）已由 implementation merge `ad8e39b23f5888cfb9a7f8f15f9dd996964d8f16`（同一 tree；parents 为精确 Base 与 Candidate）集成，post-merge CI `32939064547` 的 5 个 jobs 全部 success，并由 `AIPT-MVP-B001-CLOSEOUT-001` 关闭。交付仅为版本化声明式 Test Plan、不可变且内容寻址的 Run Manifest，以及 PostgreSQL 18.4 权威串行 Queue/Lease/Attempt 持久层；Run Core、Agent 编排、产品模型调用与真实桌测仍未实现。当前 `GLOBAL_WIP = 0`；下一项 `UNREGISTERED-AIPT-P1-B000` 仍为 `NOT_STARTED` / `NOT_AUTHORIZED`，`next_batch_authorized = false`、`next_batch_started = false`。
...
**设计基线已冻结，B001 交付已合并关闭。** M0 B001–B008、AIPT-MVP-B000 与 AIPT-MVP-B001 均已合并关闭；既有协议、SDK、账本、Runtime Shell、Harness Adapter、Evidence/Audit 与 loopback-only Web Dashboard 身份保持不可变。AIPT-MVP-B001 只新增 Test Plan / Run Manifest / Queue-Lease-Attempt 权威合同；queue backend 不等于 Run engine，且 report generator、socket/model runtime、Agent 编排与完整动作管线仍未建设。`UNREGISTERED-AIPT-P1-B000` 未获授权且未启动；`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 保持耐久名称 `AIPT M0 CI`，在 `ubuntu-24.04` 与 `ubuntu-26.04` 两个 toolchain matrix 实例无条件运行 `check:mvp-b001`、`check:mvp-bootstrap` 与 B008 `check:m0-development-pass`，并保留全部历史聚焦门。`pnpm run check` 是历史门与 AIPT-MVP-B001 的聚合门禁（报告 schema `aipt.public.mvp-b001-validator-run/v1`）。
- 存储（B003 + MVP-B001）：[docs/storage/README.md](docs/storage/README.md)（冻结 `000001_ledger.sql` 精确校验和，仅新增 `000002_playtest_queue.sql`；不可变 Manifest、确定性优先级/能力/依赖选择、数据库时间 Lease 心跳/过期/恢复、PostgreSQL 唯一约束实施 formal WIP=1、append-only Attempt 历史；PostgreSQL 18.4 全量与 race 集成门禁；机器门禁 `pnpm run check:storage`）
