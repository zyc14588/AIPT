...
## 当前状态（as of 2026-08-30）

| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IN_PROGRESS`，`current_batch = AIPT-MVP-B004`，`GLOBAL_WIP = 1`；施工快照 `AIPT-MVP-B004-CONSTRUCTION-001` |
...
`AIPT-MVP-B003` 现为 **MERGED_CLOSED**：唯一授权 Candidate `4f2979f4495e3d78393e9f9ec1978308a7fb10b9`（tree `bbdb35861bb6cf47a9e6ec4e943720c0126cbc50`，Candidate CI `33247140362` success）由合法 no-ff merge `beb7c70738b1f876845d68bec8e20166ab3eac10` 精确集成，post-merge CI `33264083089` 的 5 个 jobs 全部 success，并由 accepted append-only `MERGED → POST_MERGE_VERIFIED → CLOSED` lifecycle chain 关闭。关闭交付只包含 provider-neutral Deterministic Agent Orchestrator：固定 `1 GM + 4 Player`、Run-bound Sessions、Persona/Character/GM Profile、Visibility/ACL-before-retrieval、Context Bundle/hash、确定性 floor、结构化 action 与有限 retry/recovery；状态变更只经冻结的 B002 Run Core。B003 的业务语义继续冻结。

`AIPT-MVP-B004` 现为 **IN_PROGRESS**：Owner 已授权从精确 B003 closeout Base `98591311c4872cdc5f091e23fba1acb500ad4599`（tree `a02ffea60c59d7187975f175875fa108c78d3cac`）施工。当前实现版本化 Model/Sampling Profile、完整 execution tuple、per-role immutable Manifest binding、write-only credential broker、双层 egress、确定性 context reduction、held-file verified assets、受管 `llama.cpp` 私有 user/network namespace，以及只通过固定单文件运行时闭包的 DeepSeek Harness `HARNESS-01`（`dsh-v0.1.0-rc.8` / `141eb6fef83422698aef7a981029e843e8161534`）真实 product gateway；backend 闭集为 `REMOTE_DEEPSEEK` 与 `LOCAL_LLAMACPP`。安全修复后的最小 `REMOTE_DEEPSEEK` 与 `LOCAL_LLAMACPP` controlled re-certification 均已 PASS，最终公开证据分别见 [remote-deepseek-controlled-real-02.json](docs/model-certification/remote-deepseek-controlled-real-02.json) 与 [local-llamacpp-controlled-real-02.json](docs/model-certification/local-llamacpp-controlled-real-02.json)；旧 `-01` 证据保留为 `SUPERSEDED_NON_FINAL`。受控流程累计 5 次真实模型调用（remote/network 3 次、local 2 次）；修复后 re-certification 精确新增 remote 与 local 各 1 次。公共 CI 仍只使用 synthetic/fake 资产，真实模型/网络调用和 secret requirement 均为 0。`GGUF-04` locator 已按 Owner 批准 root、canonical target、完整 SHA-256 与 GGUF metadata 失败关闭地验证，locator 不进入公开 evidence；`LLAMACPP-01` executable/GGUF 同对象消费、私有 namespace、Harness-routed minimum role invocation 与有界生命周期探针均 PASS，且未改变 no-API-key authority。上一 Candidate `abd684a4d858376866766d67653f212c26ca4215` / `0141bb24f7c46cfcc3d0ce0a50b17a0adf631d93` 已标记 `REJECTED_PRE_PUSH_SECURITY_RESCAN`。MODEL/HARNESS launcher gate 已接线，runtime 仍 `runtime_ready = false`，首个未实现 gate 为 `IPC`；`DEFER-003` 继续延期。没有真实 playtest 或 qualification Run；替代 Candidate 必须使用新的 commit/tree 且重新取得公开披露授权，未 merge、未 closeout，下一串行批次未授权且未启动。

M0 未执行真实 TRPG 桌测，也不是 MVP Development Pass。`production_qualification = NOT_GRANTED`、`release_qualification = NOT_GRANTED`、`mvp_development_pass = NOT_GRANTED`、`human_equivalence = NOT_CLAIMED`、`real_playtest_completion = NOT_CLAIMED`；第二审计者生产门仍未完成，IPC 生产 gate 仍未实现。`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
**M0-B004 Runtime Shell** 已作为关闭历史保存在 accepted implementation merge `d07c0c3817620ada47b3ae7344d8ee423ace3b12`（tree `f35365d0ad47fdd513fbecb84a03b1559026637e`）：Go Launcher 固定执行 `CONFIG → POSTGRESQL → MIGRATIONS → MODEL → HARNESS → CORE → IPC → WEB`。MVP-B004 施工在不改顺序的前提下为 MODEL/HARNESS 接入受治理 runtime；IPC 继续失败关闭，后续 WEB 不能绕过它，计划仍不能冒充 runtime ready。共享配置、Core lifecycle、逆序有界清理及 M0-B004 历史保持不变。运行与 Web 边界见 [`docs/runtime/README.md`](docs/runtime/README.md)。
...
**设计基线保持冻结，B003 Agent Orchestrator 已关闭，B004 正在施工。** M0 B001–B008、AIPT-MVP-B000、AIPT-MVP-B001、AIPT-MVP-B002、AIPT-MVP-B003 与外部 P1 前序均为不可变关闭历史。B004 不执行真实桌测、qualification Run、运行控制 UI 或审计生成器；当前受控真实模型调用仍为 0。`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 保持耐久名称 `AIPT M0 CI`，在 `ubuntu-24.04` 与 `ubuntu-26.04` 两个 toolchain matrix 实例运行 B002/B003 冻结回归和 B004 lifecycle/profile/gateway synthetic unit/race 门禁；独立 PostgreSQL job继续使用 loopback-only 18.4 临时容器执行 Run Core 全量与 race 集成。公共 CI 不绑定 secret、不调用真实 provider/model；`pnpm run check` 报告 schema 为 `aipt.public.mvp-b004-validator-run/v1`。
