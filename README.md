...
## 当前状态（as of 2026-08-22）

| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；施工状态 `IN_PROGRESS`，`current_batch = AIPT-M0-B005`，`GLOBAL_WIP = 1`；权威快照 `AIPT-M0-B005-CONSTRUCTION-001` |
...
`AIPT-M0-B005` 正在施工：基线为 B004 closeout `8005dd3bec8b367a6d97dcd9397158f1d8618f3e`（tree `d0f32b7ac1c3f6e5ddb258aaa2ee030844b1eb2b`），交付范围严格限定为 Harness Adapter、stdio 运行时、确定性最小夹具 backend、真实子进程 smoke 与相应 CI/供应链门禁。当前 `current_batch = AIPT-M0-B005`、`GLOBAL_WIP = 1`、施工状态 `IN_PROGRESS`。下一串行批次 `AIPT-M0-B006` 为 `NOT_AUTHORIZED`（`next_batch_authorized = false`，`next_batch_started = false`），本批次不得建设 Evidence/Audit Schema 或导出层。外部 DSH 兼容目标经 Owner 升级授权 `AIPT-M0-B005-EXTERNAL-HARNESS-UPGRADE-001` 固定为 `dsh-v0.1.0-rc.8` / `141eb6fef83422698aef7a981029e843e8161534`，旧提交 `47f943859bef60e4160492346772ded9b24f765a` 保留为历史 provenance。

**B005 Harness Adapter（施工中）** 是一方薄适配层：复用 `@aipt/adapter-sdk` 的既有 wire truth，以每行一个 UTF-8 JSON-RPC 信封的 stdio 边界连接可注入的 `HarnessBackend`；stdout 只承载协议帧，stderr 只承载脱敏诊断。测试专用 fixture backend 从 B002 权威夹具读取 ACCEPT/REJECT 输出，不在生产源码硬编码游戏字段，也不发起网络或模型调用。详见 [docs/harness/README.md](docs/harness/README.md)（随本批次施工）。
...
- Harness Adapter（B005，施工中）：[packages/harness-adapter/README.md](packages/harness-adapter/README.md)（`@aipt/harness-adapter@0.1.0`，仅依赖一方 `@aipt/adapter-sdk`；受控 stdio、可注入 backend、真实子进程 smoke；机器门禁 `pnpm run check:harness-adapter`，测试 `pnpm run test:harness-adapter`）
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 以耐久名称 `AIPT M0 CI` 在 `ubuntu-24.04` 与 `ubuntu-26.04` 两个合格 runner 上保留 B002 聚焦命令，并显式增加 B005 的 `pnpm run check:harness-adapter` 与 `pnpm run test:harness-adapter`；`pnpm run check` 是 B001–B005 聚合门禁（报告 schema `aipt.public.b005-validator-run/v1`）
