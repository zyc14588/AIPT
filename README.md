...
| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IN_PROGRESS`，`current_batch = AIPT-MVP-B002`，`GLOBAL_WIP = 1`；候选状态快照 `AIPT-MVP-B002-CANDIDATE-STATUS-001` |
...
外部串行前序 `UNREGISTERED-AIPT-P1-B000` 已由 accepted merge `fe0965977447caf8cd7b6e58252bc1b991b7cc6f`、post-merge CI `33186880614` success 和 AIPT canonical lifecycle closeout `411bf2997cd0f10ba1a022ac687d27a1bd19eb36` 解析为 **MERGED_CLOSED**。`AIPT-MVP-B002` 现为唯一活跃批次：精确 Base `411bf2997cd0f10ba1a022ac687d27a1bd19eb36`（tree `d1daaeede13a2ba07c3b528c1792ef9fd5600a63`），分支 `task/AIPT-MVP-B002`，`GLOBAL_WIP = 1`。本 Candidate 只实现 game-neutral Deterministic Run Core：fail-closed action transaction、不可变 Run binding、版本化分域 RNG 与 seed commitment、PostgreSQL append-only ledger 原子提交、派生 projection 及严格 replay；`merge_authorized = false`、`closeout_authorized = false`。`AIPT-MVP-B003` 保持 `NOT_STARTED` / `NOT_AUTHORIZED`。
...
**设计基线保持冻结，B002 只施工 Run Core。** M0 B001–B008、AIPT-MVP-B000、AIPT-MVP-B001 与外部 P1 前序均为不可变关闭历史。B002 不实现 Agent 编排、持久 Agent session、模型 gateway、产品模型调用、真实桌测、qualification Run、运行控制 UI 或审计生成器；`real_model_calls = 0`、`real_playtest_executed = false`。`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 保持耐久名称 `AIPT M0 CI`，在 `ubuntu-24.04` 与 `ubuntu-26.04` 两个 toolchain matrix 实例运行 B002 独立门禁与 Run Core 测试，同时在精确 B002 Base 上重放不可变 B001 gate；独立 PostgreSQL job 使用 loopback-only 18.4 临时容器执行 Run Core 全量与 race 集成。`pnpm run check` 报告 schema 为 `aipt.public.mvp-b002-validator-run/v1`。
