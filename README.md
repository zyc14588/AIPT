...
`AIPT-MVP-B004` 现为 **IN_PROGRESS**：Owner 已授权从精确 B003 closeout Base `98591311c4872cdc5f091e23fba1acb500ad4599`（tree `a02ffea60c59d7187975f175875fa108c78d3cac`）施工。当前实现版本化 Model/Sampling Profile、完整 execution tuple、per-role immutable Manifest binding、write-only credential broker、双层 egress、确定性 context reduction、受管 `llama.cpp` loopback runtime，以及只通过固定 DeepSeek Harness `HARNESS-01`（`dsh-v0.1.0-rc.8` / `141eb6fef83422698aef7a981029e843e8161534`）的真实 product gateway；backend 闭集为 `REMOTE_DEEPSEEK` 与 `LOCAL_LLAMACPP`。Owner 批准的 credential reference 已绑定，Harness route 已冻结；受控 `REMOTE_DEEPSEEK` 与 `LOCAL_LLAMACPP` minimum certification 均已 PASS，公开且无秘密的证据分别见 [remote-deepseek-controlled-real-01.json](docs/model-certification/remote-deepseek-controlled-real-01.json) 与 [local-llamacpp-controlled-real-01.json](docs/model-certification/local-llamacpp-controlled-real-01.json)。受控流程累计 3 次真实模型调用：2 次为既有 remote/network 调用（formal remote certification 精确绑定其中 1 次成功调用），1 次为 local 成功调用；local 首次 readiness 失败发生在调用前。公共 CI 仍只使用 synthetic/fake 资产，真实模型/网络调用和 secret requirement 均为 0。`GGUF-04` locator 已按 Owner 批准 root、canonical target、完整 SHA-256 与 GGUF metadata 失败关闭地验证，locator 不进入公开 evidence；`LLAMACPP-01` executable compatibility、受管启动、Harness-routed minimum role invocation 与有界关闭/失败探针均 PASS，且没有下载、猜测或替代资产。MODEL/HARNESS launcher gate 已接线，runtime 仍 `runtime_ready = false`，首个未实现 gate 为 `IPC`；`DEFER-003` 继续延期。没有真实 playtest 或 qualification Run；Candidate 身份由最终提交/Tree 与公开 CI Stage Review 确定，未 merge、未 closeout，下一串行批次未授权且未启动。
