...
## 当前状态（as of 2026-08-25）

| 工作轨 | 状态 |
|---|---|
| `AIPT-STANDALONE` | 设计已冻结（`FROZEN_R0_R16_DCA_BOOTSTRAP`）；`construction = IN_PROGRESS`，`current_batch = AIPT-M0-B008`，`GLOBAL_WIP = 1`；权威快照 `AIPT-M0-B008-CANDIDATE-001` |
...
`AIPT-M0-B008` = **IN_PROGRESS Candidate**：独立 GPT M0 development audit 为 `PASS` 且 open findings 为空；当前只提出 `M0_DEVELOPMENT_PASS`，其状态严格为 `PROPOSED_PENDING_B008_MERGED_CLOSED` / `NOT_YET_GRANTED`，仅在 B008 未来经 Owner 授权完成 Merge 与 closeout 后才可能生效。accepted M0 implementation 仍固定为 `e05179a223f9dd0ff1b317e78c0e466e1146f6bb`（tree `35a5cc261fef75df8d25102015670bcb1d6fbd92`），B008 不修改产品运行时。当前 `next_serial_batch = NONE`、`next_batch_state = NOT_AUTHORIZED`、`next_batch_authorized = false`、`next_batch_started = false`；`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。详见 [M0 Development Pass Candidate](docs/milestones/M0_DEVELOPMENT_PASS.md)。
...
**设计基线已冻结。** B001–B007 均已合并关闭；协议、SDK、PostgreSQL 存储、Runtime Shell、Harness Adapter、Evidence/Audit 与安全的 loopback-only Web Dashboard 实现身份均保持不可变。B008 只构建里程碑 Candidate 与机器门禁，不建设 queue/run/status backend、report generator、socket/model runtime 或完整动作管线；`AIPT-PLATFORM-INTEGRATION` 保持 `FROZEN_WAITING_M1_ENGINE`。
...
- 里程碑合同：[docs/milestones/M0.md](docs/milestones/M0.md)、[M0 Development Pass Candidate](docs/milestones/M0_DEVELOPMENT_PASS.md) 与 [docs/milestones/MVP.md](docs/milestones/MVP.md)
...
- 公共 CI：[.github/workflows/ci.yml](.github/workflows/ci.yml) 以耐久名称 `AIPT M0 CI` 在 `ubuntu-24.04` 与 `ubuntu-26.04` 两个合格 runner 上保留全部既有聚焦命令，并在两个 toolchain matrix 实例显式运行 B008 `check:m0-development-pass`；B007 Web 门与 digest-pinned、loopback-only PostgreSQL 18.4 集成/race 全部保留。`pnpm run check` 是 B001–B008 聚合门禁（报告 schema `aipt.public.b008-validator-run/v1`）。
