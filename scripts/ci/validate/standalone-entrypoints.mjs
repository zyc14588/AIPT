...
export function run(ctx) {
  ...
  const positives = [
    ...
    { name: 'historical-governance', entrypoint: 'historical-governance', label: 'status-transition and tree-integrity at exact M0 closeout', args: ['--repo', ctx.repo, '--gate', 'm0-development-pass'], result: 'PASS' },
    ...
    { name: 'web-ui', entrypoint: 'mvp-b001', label: 'web-ui exact historical gate on frozen B001 Base', args: ['--repo', ctx.repo, '--historical-web'], result: 'PASS' },
    {
      ...
      entrypoint: 'historical-governance',
      label: 'm0-development-pass final M0 gate at exact closeout',
      args: ['--repo', ctx.repo, '--gate', 'm0-development-pass'],
      result: 'PASS',
    },
    {
      name: 'historical-governance',
      entrypoint: 'historical-governance',
      label: 'mvp-bootstrap B000 preservation at exact closeout',
      args: ['--repo', ctx.repo, '--gate', 'mvp-bootstrap'],
      result: 'PASS',
    },
    {
      name: 'mvp-b001',
    ...
    {
      name: 'p1-b000-authority-amendment',
      label: 'accepted P1 B000 Authority Amendment provenance gate',
      args: ['--repo', ctx.repo],
      result: 'PASS',
    },
    {
      name: 'p1-b000-authority-repair',
      label: 'P1 B000 Authority repair and supersession gate',
      args: ['--repo', ctx.repo],
      result: 'PASS',
    },
    {
