...
export function validateSbomSemantics(doc, { repo, toolchainLock, actionsLock }) {
  ...
  if (webPkg) {
    if (webPkg.versionInfo !== '0.1.0') {
      fail(`@aipt/web-ui versionInfo must be 0.1.0, got ${JSON.stringify(webPkg.versionInfo)}`);
    } else ok('@aipt/web-ui versionInfo = 0.1.0');
    const webPurl = (webPkg.externalRefs ?? []).find((r) => r?.referenceType === 'purl');
    if (!webPurl || webPurl.referenceLocator !== WEB_UI_NPM_PURL) {
      fail(`@aipt/web-ui npm purl must be exactly ${WEB_UI_NPM_PURL}`);
    } else ok('@aipt/web-ui carries the exact npm purl');
    const comment = webPkg.comment ?? '';
    if (!comment.includes('dependency-free') || !comment.includes('AIPT/root MIT license record') ||
        !comment.includes('PACKAGE_OF') || !comment.includes('zero npm registry') ||
        !comment.includes('never classified as DEV_TOOL_OF')) {
      fail('@aipt/web-ui comment must document root MIT coverage, dependency-free PACKAGE_OF and zero registry identity');
    } else ok('@aipt/web-ui comment records exact B007 first-party dependency-free coverage');
  }
  const webPackageOf = doc.relationships.some(
    (r) => r.spdxElementId === WEB_UI_SPDXID && r.relationshipType === 'PACKAGE_OF' && r.relatedSpdxElement === AIPT_SPDXID,
  );
  if (!webPackageOf) fail(`missing first-party relationship: ${WEB_UI_SPDXID} PACKAGE_OF ${AIPT_SPDXID}`);
  else ok('first-party relationship present: SPDXRef-web-ui PACKAGE_OF SPDXRef-AIPT');
  if (doc.relationships.some(
    (r) => r.spdxElementId === WEB_UI_SPDXID && r.relationshipType === 'DEV_TOOL_OF' && r.relatedSpdxElement === AIPT_SPDXID,
  )) fail(`${WEB_UI_SPDXID} must never be DEV_TOOL_OF AIPT`);
  else ok('@aipt/web-ui is never classified as DEV_TOOL_OF');
...
export function run(ctx) {
  ...
  const webPackageProbes = [
    {
      label: 'Web UI package deleted from the SBOM',
      reason: /required package missing: @aipt\/web-ui/,
      mutate: (probeDoc) => {
        probeDoc.packages = probeDoc.packages.filter((p) => p.SPDXID !== WEB_UI_SPDXID);
      },
    },
    {
      label: 'Web UI package wrongly licensed',
      reason: /MIT/,
      mutate: (probeDoc) => {
        const p = probeDoc.packages.find((x) => x.SPDXID === WEB_UI_SPDXID);
        p.licenseConcluded = 'Apache-2.0';
        p.licenseDeclared = 'Apache-2.0';
      },
    },
    {
      label: 'Web UI npm purl drifted',
      reason: /web-ui npm purl/,
      mutate: (probeDoc) => {
        const p = probeDoc.packages.find((x) => x.SPDXID === WEB_UI_SPDXID);
        p.externalRefs[0].referenceLocator = 'pkg:npm/%40aipt/web-ui@9.9.9';
      },
    },
    {
      label: 'Web UI PACKAGE_OF relationship deleted',
      reason: /missing first-party relationship: SPDXRef-web-ui PACKAGE_OF SPDXRef-AIPT/,
      mutate: (probeDoc) => {
        probeDoc.relationships = probeDoc.relationships.filter(
          (r) => !(r.spdxElementId === WEB_UI_SPDXID && r.relationshipType === 'PACKAGE_OF'),
        );
      },
    },
    {
      label: 'Web UI PACKAGE_OF relationship retyped to DEV_TOOL_OF',
      reason: /missing first-party relationship: SPDXRef-web-ui PACKAGE_OF SPDXRef-AIPT|must never be DEV_TOOL_OF/,
      mutate: (probeDoc) => {
        const rel = probeDoc.relationships.find(
          (r) => r.spdxElementId === WEB_UI_SPDXID && r.relationshipType === 'PACKAGE_OF',
        );
  ...
  for (const def of webPackageProbes) {
    const probeDoc = JSON.parse(JSON.stringify(doc));
    if (!probeDoc.packages.some((x) => x.SPDXID === WEB_UI_SPDXID)) {
      fail(`negative ${def.label} probe could not run: Web UI package missing`);
      continue;
    }
    def.mutate(probeDoc);
    const probeResult = validateSbomSemantics(probeDoc, { repo: ctx.repo, toolchainLock, actionsLock });
    if (probeResult.result !== 'FAIL') fail(`negative ${def.label} probe was NOT rejected`);
      ...
      fail(`negative ${def.label} probe failed for an unexpected reason`);
    } else ok(`negative-probe PASS: ${def.label} rejected by the B007 Web UI first-party model`);
  }
