...
export class DashboardShapeError extends Error {
           code = 'AIPT_WEB_INVALID_DASHBOARD';
...
function exactString(record            , key        , expected        )         {
  const value = record[key];
  if (value !== expected) fail();
  return value;
}

function boundedString(record            , key        , maximum        , pattern        )         {
  const value = record[key];
  if (typeof value !== 'string' || value.length < 1 || value.length > maximum || !pattern.test(value)) fail();
  return value;
}

function exactEmptyArray(record            , key        )     {
  const value = record[key];
...
function parseConfig(value         )              {
  ...
  return {
    schema: exactString(record, 'schema', 'aipt.config/v1')                    ,
    profile,
...
function parseHealth(value         )              {
  ...
  return {
    serving_status: exactString(record, 'serving_status', 'SERVING')             ,
    runtime_readiness: exactString(record, 'runtime_readiness', 'NOT_ASSERTED')                  ,
  };
}
...
function parseQueue(value         )             {
  ...
  return {
    backend_authority: exactString(record, 'backend_authority', 'POSTGRESQL')                ,
    implementation_status: exactString(record, 'implementation_status', NOT_IMPLEMENTED)                     ,
...
function parseStatusTable(value         )                   {
...
function parseReport(value         )              {
  ...
  return {
    raw_capture: exactString(record, 'raw_capture', 'IMPLEMENTED_LIBRARY_ONLY')                              ,
    ui_export_action: exactString(record, 'ui_export_action', NOT_IMPLEMENTED)                     ,
    audit_ready_generator: exactString(record, 'audit_ready_generator', NOT_IMPLEMENTED)                     ,
    audit_result_generator: exactString(record, 'audit_result_generator', NOT_IMPLEMENTED)                     ,
    signing: exactString(record, 'signing', NOT_IMPLEMENTED)                     ,
    encryption: exactString(record, 'encryption', NOT_IMPLEMENTED)                     ,
    chunking: exactString(record, 'chunking', NOT_IMPLEMENTED)                     ,
  };
}
...
export function parseDashboard(value         )                    {
  const root = exactObject(value, ROOT_KEYS);
  return {
    schema: exactString(root, 'schema', DASHBOARD_SCHEMA)                           ,
...
function element(documentLike              , tagName        , className        , text        )              {
...
function row(documentLike              , label        , value        )              {
...
function panel(documentLike              , title        , variant        , rows                             )              {
...
export function renderDashboard(root             , snapshot                   , documentLike              )       {
